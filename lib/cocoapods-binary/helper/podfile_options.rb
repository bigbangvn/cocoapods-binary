...
module Pod
    class Installer
        ...
        def prebuild_pod_names
            # Bang: testing
           @prebuild_pod_names ||= self.prebuild_pod_targets.map(&:pod_name).reject { |name| Prebuild::DaxIOSWorkaround.localResourcesTargets.include?(name) || Prebuild::DaxIOSWorkaround.issueTargets.include?(name) || Prebuild::DaxIOSWorkaround.podsBlackList.include?(name) }.reject { |name| sandbox.local?(name) && !Prebuild::DaxIOSWorkaround.devpodWhiteList.include?(name) }
        end
